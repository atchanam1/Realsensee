...
export default function LoginPage() {
  ...
  if (status === 'authenticated' && !session.user.isApproved) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0f0f1a]">
        <div className="bg-[#1a1a2e] border border-[#2d2d4e] rounded-2xl p-10 max-w-sm w-full text-center shadow-2xl">
          <div className="text-5xl mb-4">⏳</div>
          <h2 className="text-xl font-bold text-white mb-2">รอการอนุมัติ</h2>
          <p className="text-gray-400 text-sm mb-4">
            บัญชีของคุณ <span className="text-purple-400 font-semibold">{session.user.username}</span><br />
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f0f1a] relative overflow-hidden">
      ...
      <div className="relative bg-[#1a1a2e] border border-[#2d2d4e] rounded-2xl p-10 max-w-sm w-full text-center shadow-2xl">
        {/* Logo */}
        <div className="text-6xl mb-4">⚔️</div>
        <h1 className="text-3xl font-bold text-white mb-1">Gang System</h1>
        <p className="text-gray-400 text-sm mb-8">ระบบจัดการแก๊ง</p>

        <button
          onClick={() => signIn('discord')}
          className="w-full flex items-center justify-center gap-3 bg-[#5865F2] hover:bg-[#4752C4] text-white font-semibold py-3 px-6 rounded-xl transition-all duration-200 shadow-lg hover:shadow-indigo-500/25 hover:scale-105"
        >
        ...
        <p className="text-gray-600 text-xs mt-6">
          เข้าสู่ระบบได้เฉพาะสมาชิกที่ได้รับอนุมัติ
        </p>
      </div>
    </div>
  )
